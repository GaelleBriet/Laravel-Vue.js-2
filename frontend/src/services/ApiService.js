const ApiService = {
    async fetchAll(type) {
        try {
            const response = await fetch(`http://pabloprevot-server.eddi.cloud:8090/api/${type}`);
    ...
    async fetchOne(type, id) {
        try {
            const response = await fetch(`http://pabloprevot-server.eddi.cloud:8090/api/${type}/${id}`);
            const data = await response.json();
            return data;
          } catch (error) {
